...
class unetCv2(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm, kernel=4, stride=2, pad=1):
        super(unetCv2, self).__init__()

        if is_batchnorm:
            self.conv1 = nn.Sequential(nn.Conv2d(in_size, out_size, kernel, stride, pad, bias=False),
                                       nn.BatchNorm2d(out_size),
                                       nn.LeakyReLU(0.2, inplace=True),)
            # self.conv2 = nn.Sequential(nn.Conv2d(out_size, out_size, kernel, stride, pad, bias=False),
            #                            nn.BatchNorm2d(out_size),
            #                            nn.ReLU(),)
        else:
            self.conv1 = nn.Sequential(nn.Conv2d(in_size, out_size, kernel, stride, pad, bias=False),
...
class unetUp(nn.Module):
    def __init__(self, in_size, out_size, is_deconv, kernel=4, stride=2, pad=1):
        super(unetUp, self).__init__()
        self.conv = unetCv2(in_size, out_size, False, 3, 1, 1)
        # self.conv = nn.Sequential(nn.ConvTranspose2d(in_size, out_size, 3, 1, 1, bias=False),nn.BatchNorm2d(out_size),nn.Dropout(p=0.5),nn.ReLU(),)
        if is_deconv:
            self.up = nn.Sequential(nn.ConvTranspose2d(in_size, out_size, kernel, stride, pad, bias=False),
                                    nn.BatchNorm2d(out_size),
                                    nn.Dropout(p=0.5),
                                    nn.ReLU(),)
            # self.up = nn.ConvTranspose2d(in_size, out_size, kernel, stride, pad, bias=False)
        else:
