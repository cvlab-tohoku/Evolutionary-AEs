...
class Individual(object):
    ...
    def check_pool(self):
        ...
        for n in range(self.net_info.node_num + self.net_info.out_num):
            if self.is_active[n]:
                if self.gene[n][0] == 6 or self.gene[n][0] == 7:
                    is_pool = False
                    pool_num += 1
...
class CGP(object):
    ...
    def modified_evolution(self, max_eval=100, mutation_rate=0.01, log_file='./log.txt', arch_file='./arch.txt'):
        with open(log_file,'w') as fw, open(arch_file, 'w') as fw_a:
            writer = csv.writer(fw, lineterminator='\n')
    ...
    def modified_evolution_full(self, max_eval=100, mutation_rate=0.01, log_file='./log.txt', arch_file='./arch.txt'):
        with open(log_file,'w') as fw, open(arch_file, 'w') as fw_a:
            writer = csv.writer(fw, lineterminator='\n')
            writer_a = csv.writer(fw_a, lineterminator='\n')
            start_time = time.time()
