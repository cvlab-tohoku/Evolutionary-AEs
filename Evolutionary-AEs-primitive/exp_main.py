...
if __name__ == '__main__':
    ...
    if args.mode == 'evolution':
        ...
        network_info = func_set[args.func_set](rows=1, cols=20, level_back=5, min_active_num=2, max_active_num=30)
        ...
        imgSize = 160
        eval_f = CNNEvaluation(gpu_num=args.gpu_num, dataset='bsds', valid_data_ratio=0.1, verbose=True, epoch_num=5, batchsize=16, imgSize=imgSize)

        # Execute evolution
        init = False
        if args.func_set == 'Primitive':
            init = True
        cgp = CGP(network_info, eval_f, lam=args.lam, imgSize=imgSize, init=init)
